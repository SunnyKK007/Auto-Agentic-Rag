...
def _get_drive_service():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # Local OAuth flow crashes on headless servers (Docker, Fly.io, CI).
            # We catch it to advise generating token.json locally first.
            try:
                flow = InstalledAppFlow.from_client_secrets_file(cred_path, SCOPES)
                creds = flow.run_local_server(port=0)
            except Exception as e:
                raise Exception(
                    "Google Drive OAuth requires an interactive browser to complete the first-time "
                    "authorisation flow. This fails on headless servers (Docker, Fly.io, CI).\n"
                    "To fix this:\n"
                    "  1. Run `python backend/drive_loader.py` on your LOCAL machine to generate "
                    "token.json.\n"
                    "  2. Commit or mount token.json to your deployment at backend/token.json.\n"
                    f"Original error: {e}"
                ) from e
...
def load_from_google_drive(url_or_id: str) -> List[Document]:
    ...
    if is_folder:
        ...
        # Paginate to ensure folders with >100 files are not truncated.
        files: list = []
        ...
        if not files:
            raise ValueError(
                f"No files found in Drive folder: {resolved_id}. "
