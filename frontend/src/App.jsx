...
export default function App() {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: 'System ready. Upload documents to begin.' }
  ]);
  ...
  const processFiles = async (selectedFiles) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (clearPrevious) {
          setMessages([{ role: 'assistant', content: 'Documents ingested. System ready.' }]);
        }
      } else {
  ...
  const handleDriveUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (clearPrevious) {
          setMessages([{ role: 'assistant', content: 'Drive folder ingested. System ready.' }]);
        }
        setDriveFolderId('');
      } else {
        const errorData = await response.json();
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = isQuotaError
          ? 'Token limit reached. Retry later.'
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between text-sm">
              <span className="text-zinc-600 flex items-center gap-2">
                {/* FIX: dot colour driven by real /api/health ping */}
              ...
              <span className="font-mono text-xs text-zinc-500">
                {apiStatus === 'online' ? 'Active' : apiStatus === 'offline' ? 'Offline' : 'Checking...'}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-zinc-600 flex items-center gap-2">
      ...
      <main className="flex-1 flex flex-col h-screen relative bg-white">
        ...
        {messages.length === 1 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-3xl mx-auto w-full text-center animate-fade-in">
            ...
            <h2 className="text-2xl font-semibold text-zinc-900 mb-2">System Ready</h2>
            <p className="text-zinc-500 mb-8 text-sm">Upload documents to begin.</p>
          </div>
        ) : (
          ...
          <form onSubmit={handleQuery} className="max-w-4xl mx-auto relative">
            <div className="relative flex flex-col bg-zinc-50 border border-zinc-200 rounded-2xl focus-within:border-zinc-400 focus-within:ring-4 focus-within:ring-zinc-100 transition-all overflow-hidden shadow-sm">
              <textarea
                ...
                placeholder={isIngesting ? "Ingesting..." : "Enter query..."}
            ...
            <div className="text-center mt-3">
              <span className="text-[11px] text-zinc-400">Verify AI output.</span>
            </div>
          </form>
        </div>
